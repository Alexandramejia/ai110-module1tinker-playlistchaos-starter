...
def compute_playlist_stats(playlists: PlaylistMap) -> Dict[str, object]:
    ...
    # fixed: was len(hype), now uses all songs
    total = len(all_songs)
    hype_ratio = len(hype) / total if total > 0 else 0.0

    avg_energy = 0.0
    if all_songs:
        # fixed: was summing only hype songs
...
def most_common_artist(songs: List[Song]) -> Tuple[str, int]:
    ...
    # max() directly finds the highest count without sorting the entire list
...
def search_songs(
    ...
    for song in songs:
        ...
        # fixed: was `value in q` (reversed), now correctly checks if query is in field value
...
def lucky_pick(
    ...
    if mode == "hype":
        ...
        # fixed: was missing Mixed playlist
